...
public function feature()
{
    ...
    $isOnSale = (int) ($product['sale_discount_percent'] ?? 0) > 5;
    $isFeatured = (int) ($product['featured'] ?? 0) === 1;

    // Prevent featuring a product that is currently on sale
    if ($isOnSale && !$isFeatured) {
        $_SESSION['flash'] = "❌ Cannot feature this product - it is currently on sale. Remove it from sale first!";
        header("Location: " . BASE_URL . "/index.php?page=admin_feature_products");
        exit;
    }
...
public function featureIndex()
{
    ...
    foreach ($allProducts as $p) {
        if ((int)$p['featured'] === 1) {
            ...
            // Can't feature products that are on sale
            if ((int)$p['sale_discount_percent'] == 0) {
    ...
    public function toggleSale()
    {
        ...
        if ($saleAction === 'add') {
            // Prevent putting featured products on sale
            if ($isFeatured) {
                $_SESSION['flash'] = "❌ Cannot put this product on sale - it is currently featured. Remove from featured first!";
                header("Location: " . BASE_URL . "/index.php?page=admin_sale_products");
                exit;
            }
        }
        ...
        if ($saleAction === 'remove') {
            ...
            header("Location: " . BASE_URL . "/index.php?page=admin_sale_products&last_discount=" . (int)$discountPercent);
            exit;
        }
                ...
                try {
                    ...
                    $mail->Body = "
                        <div style='font-family:Arial,sans-serif;padding:20px;color:#111;'>
                            ...
                            <p>{$product['details']}</p>
                            <p><a href='" . BASE_URL . "index.php?page=product_show&id={$product['id']}' style='color:#c19a4e;'>View product now</a></p>
                            <hr>
