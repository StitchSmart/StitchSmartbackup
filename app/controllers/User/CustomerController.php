    ...
    public function login() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($user) {
                ...
                $redirectUrl = $_SESSION['redirect_after_login'] ?? (BASE_URL . "index.php?page=" . ($redirect === 'customer_orders' ? 'home' : $redirect));
    ...
    public function processForgotPassword() {
        ...
        if ($step === 'request') {
            ...
            try {
                ...
                $_SESSION['forgot_success'] = "OTP sent to {$email}. Enter it below within 60 seconds.";

            } catch (Exception $e) {
                $_SESSION['forgot_error'] = "Could not send OTP email. Please try again.";
                header("Location: " . BASE_URL . "index.php?page=customer_forgot_password");
                exit;
            }
        ...
        /* ── STEP 2: Verify OTP ── */
            ...
            if (empty($entered)) {
                ...
                $_SESSION['forgot_error']        = "OTP expired. Please request a new one.";
                $_SESSION['reset_step_customer'] = 'request';
            } elseif ($entered == $saved) {
                $_SESSION['otp_verified_customer'] = true;
                $_SESSION['reset_step_customer']   = 'reset_password';
                $_SESSION['forgot_success']        = "OTP verified! Please choose a new password.";
            } else {
                $_SESSION['forgot_error'] = "Invalid OTP. Please try again.";
            }
        ...
        /* ── STEP 3: Reset Password ── */
            ...
            if (empty($_SESSION['otp_verified_customer'])) {
                $_SESSION['forgot_error']        = "Unauthorized. Please start over.";
                $_SESSION['reset_step_customer'] = 'request';
                header("Location: " . BASE_URL . "index.php?page=customer_forgot_password");
                exit;
            }
            ...
            if (strlen($password) < 4) {
                $_SESSION['forgot_error'] = "Password must be at least 4 characters.";
                ...
                if ($user && $this->userModel->updatePassword($user['id'], password_hash($password, PASSWORD_DEFAULT))) {
                    ...
                    $_SESSION['login_success'] = "Password updated successfully. You can now login.";
                    header("Location: " . BASE_URL . "index.php?page=customer_login");
                    exit;
                } else {
