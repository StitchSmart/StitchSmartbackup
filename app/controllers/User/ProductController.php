...
class ProductController {
    ...
    {
        $defaultRedirect = BASE_URL . "/index.php?page=product_show&id=" . $productId;
        ...
        if (strpos($safeRedirect, 'index.php?') === 0) {
            return BASE_URL . '/' . ltrim($safeRedirect, '/');
        }

        if (strpos($safeRedirect, '?page=') === 0) {
            return BASE_URL . '/index.php' . $safeRedirect;
        }

        if (preg_match('/^[a-zA-Z0-9_\-]+$/', $safeRedirect)) {
            return BASE_URL . '/index.php?page=' . $safeRedirect;
        }
...
public function show(){
    ...
    if(!$product){
        ...
        echo "<a href='".BASE_URL."index.php?page=products' style='display:inline-block; margin-top:30px; padding:12px 24px; background:var(--chat-accent, #1a0f0a); color:#fff; text-decoration:none; border-radius:30px; font-weight: bold; transition: 0.3s;'>Return to Shop</a>";
        echo "</div>";
    ...
    if ($userId) {
        $related_products = $this->productModel->getAiRecommendedProducts($userId, $product['id'], $product['parent_cat']);
        
        // Fallback to category related products if AI recommendation returns none
...
public function saveReview()
{
    ...
    if (!is_string($submittedToken) || !hash_equals($_SESSION['csrf_token'], $submittedToken)) {
        ...
        header("Location: " . BASE_URL . "/index.php?page=product_show&id=" . (int)($_POST['product_id'] ?? 0));
        exit;
    }
