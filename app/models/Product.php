...
public function searchProducts($keyword, $sort = null){
    ...
    $words = explode(' ', trim((string)$keyword));
    $ai_keywords = [];
    foreach ($words as $w) {
        $w = trim($w);
        if (strlen($w) > 2) {
    ...
    // If no meaningful words, fallback to exact phrase
    if (empty($ai_keywords)) {
        $ai_keywords = [trim((string)$keyword)];
    }
    ...
    foreach ($ai_keywords as $kw) {
        $likes[] = "(p.name LIKE ? OR p.description LIKE ? OR p.meta_keywords LIKE ?)";
        $kwParam = "%" . $kw . "%";
        $params[] = $kwParam;
        $params[] = $kwParam;
        $params[] = $kwParam;
