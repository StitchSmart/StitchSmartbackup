...
.prod-grid-btn {
    ...
    justify-content: center;
    cursor: pointer;
    box-shadow: 0 4px 10px rgba(193,154,78,0.3);
    transition: all 0.3s ease;
}
...
.prod-grid-btn:hover {
    background: #b08238 !important;
    transform: scale(1.1) !important;
    box-shadow: 0 6px 16px rgba(193,154,78,0.45) !important;
}
.prod-grid-card:hover .prod-grid-btn {
    background: #000;
    transform: none;
...
.prod-grid-btn:hover {
    background: var(--accent-bronze, #CD9A48) !important;
}
...
.rate-btn {
    ...
    margin-top: 12px;
    padding: 10px 0;
    border: 1.5px solid rgba(193,154,78,0.4);
    border-radius: 12px;
    background: transparent;
    color: #b08238;
    font-size: 0.83rem;
...
.rate-btn:hover {
    background: rgba(193,154,78,0.13);
    border-color: #CD9A48;
    color: #CD9A48;
}
.rate-btn i { font-size: 0.95rem; color: #CD9A48; }
.rating-panel {
    display: none;
    flex-direction: column;
    align-items: center;
    ...
    border: 1.5px solid rgba(193,154,78,0.25);
    border-radius: 16px;
...
.rp-submit {
    ...
    box-shadow: 0 3px 10px rgba(193,154,78,0.25);
}
.rp-submit.ready {
    opacity: 1;
    pointer-events: all;
...
.rp-submit.ready:hover {
    ...
    box-shadow: 0 5px 16px rgba(193,154,78,0.4);
} 
.rp-cancel {
    font-size: 0.75rem;
    color: #bbb;
...
<div class="row g-4 align-items-start" id="productsGrid">                        <!-- Products dynamically rendered here -->
                    ...
                    <!-- Empty State -->
                    <div id="emptyState" class="empty-state-container text-center py-5 d-none" style="background: rgba(255, 253, 248, 0.5); border: 1px dashed rgba(193, 154, 78, 0.4); border-radius: 20px; margin-top: 2rem;">
                        <div class="empty-state-icon mb-3">
                            <i class="bi bi-search" style="font-size: 3rem; color: rgba(193,154,78,0.6);"></i>
                        </div>
...
function applyFiltersAndRender() {
    ...
    if (titleEl) {
        if (searchVal !== '') {
            titleEl.innerHTML = `<span style="font-size:1.1rem; color:#888; font-weight:400;">Search Results for:</span> <br><span style="color:#c19a4e; font-style:italic;">"${escapeHtml(searchVal)}"</span>`;
...
function renderGrid(products) {
    ...
    products.forEach(prod => {
        ...
        const ratingDisplay = prodRating > 0
            ? `<span class="fas fa-star" style="color:#CD9A48;font-size:.85rem"></span> ${prodRating.toFixed(1)} <span style="color:#9a7522">(${reviewCount})</span>`
            : `<span style="color:#1a0f0a !important;font-size:.85rem;font-weight:700;">No ratings yet</span>`;
...
card.style.cssText = "align-items: flex-start;";
        card.innerHTML = `
            <div class="prod-grid-card" style="position:relative;">
                 <a href="${baseUrl}index.php?page=product_show&id=${prod.id}" style="text-decoration:none;">
                ...
                <div class="prod-grid-rating">
                    <div class="stars" id="stars-display-${prod.id}">
                        ${Array.from({length:5},(_,i)=>{
                            ...
                            return `<i class="${c}" style="color:#CD9A48"></i>`;
                        }).join('')}
                    </div>
                    <div id="rating-summary-${prod.id}" style="font-size:.82rem;margin-top:2px">${ratingDisplay}</div>
                </div>
                <a href="${baseUrl}index.php?page=product_show&id=${prod.id}" style="text-decoration:none; color:inherit;">
    <h3 class="prod-grid-title">${prod.name}</h3>
                ...
                <div class="prod-grid-footer">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <form method="POST" action="${baseUrl}index.php?page=cart_add" class="m-0">
                            <input type="hidden" name="product_id" value="${prod.id}">
                            <input type="hidden" name="qty" value="1">
                            <input type="hidden" name="csrf_token" value="${csrfToken}">
                            <input type="hidden" name="redirect_to" value="?page=allproducts">
                            <button type="submit" class="prod-grid-btn" title="Add to cart">
                                <i class="bi bi-cart-plus"></i>
                            </button>
                        </form>
                        <form method="POST" action="${baseUrl}index.php?page=wishlist_toggle" class="m-0">
                            <input type="hidden" name="csrf_token" value="${csrfToken}">
                            <input type="hidden" name="product_id" value="${prod.id}">
                            <input type="hidden" name="redirect_to" value="?page=allproducts">
...
function submitQuickRating(pid) {
    ...
    fetch(baseUrl + 'index.php?page=quick_rate', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                ...
                if (summaryEl) {
                    summaryEl.innerHTML = `<span class="fas fa-star" style="color:#CD9A48;font-size:.85rem"></span> ${avg.toFixed(1)} <span style="color:#aaa">(${cnt})</span>`;
                }
