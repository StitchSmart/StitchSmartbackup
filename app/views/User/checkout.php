...
.checkout-primary-btn {
  background: linear-gradient(135deg, var(--lux-gold) 0%, #a67c37 100%) !important;
...
.checkout-primary-btn:hover {
  ...
  box-shadow: 0 12px 25px rgba(193, 154, 78, 0.3) !important;
  color: #ffffff !important;
}
  ...
  <div class="row g-5">
    ...
    <div class="col-lg-7 animate-fade-in-up delay-1">
      <div class="premium-card">
        ...
        <form id="checkoutForm" method="POST" action="<?= BASE_URL; ?>/index.php?page=place_order">
          ...
          <div class="voucher-wrapper">
            ...
            <div class="input-group">
              <input type="text" id="voucher_code" name="voucher_code" class="form-control form-control-custom" placeholder="Enter code (e.g. STITCH20)">
              <button type="button" id="applyVoucherBtn" class="btn btn-outline-warning px-4" style="border-radius: 0 12px 12px 0;">Apply</button>
            </div>
                    ...
                    <div class="tab-pane fade show active" id="signin" role="tabpanel">
                        <form method="POST" action="<?= BASE_URL; ?>index.php?page=customer_login">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a href="<?= BASE_URL; ?>index.php?page=customer_forgot_password" class="checkout-link text-warning">Forgot?</a>
                            </div>
                        </form>
                    </div>
...
<div id="jazzcashModal" style="display:none;position:fixed;inset:0;z-index:99999;background:rgba(30,15,5,0.65);backdrop-filter:blur(8px);align-items:center;justify-content:center;">
  <div id="jcCard" style="background:#fff;border-radius:28px;max-width:430px;width:94%;box-shadow:0 40px 100px rgba(92,67,53,0.30);overflow:hidden;animation:jcSlideUp 0.45s cubic-bezier(0.16,1,0.3,1);">
    ...
    <div style="background:linear-gradient(135deg,#CD9A48 0%,#8b5a2b 100%);padding:22px 26px 18px;display:flex;align-items:center;justify-content:space-between;">
    ...
    <div id="jcScreenRegister" style="display:none;padding:28px;">
      <div style="text-align:center;margin-bottom:20px;">
        <div style="font-size:0.72rem;color:#CD9A48;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;">Create Account</div>
      ...
      <div style="margin-bottom:15px;">
        ...
        <div style="position:relative;">
          <span style="position:absolute;left:13px;top:50%;transform:translateY(-50%);color:#CD9A48;font-weight:700;font-size:0.88rem;">&#127477;&#127472; +92</span>
          <input id="regMobile" type="tel" maxlength="10" placeholder="3XXXXXXXXX" style="width:100%;border:1.5px solid #E6DCD2;border-radius:13px;padding:13px 13px 13px 74px;font-size:0.93rem;outline:none;color:#3D241C;background:#FFFDF9;box-sizing:border-box;" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.12)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
        </div>
      </div>
      <div style="margin-bottom:15px;">
        ...
        <div style="position:relative;">
          <input id="regMpin" type="password" maxlength="6" placeholder="&bull;&bull;&bull;&bull;&bull;&bull;" style="width:100%;border:1.5px solid #E6DCD2;border-radius:13px;padding:13px 40px 13px 16px;font-size:1.2rem;letter-spacing:7px;outline:none;color:#3D241C;background:#FFFDF9;box-sizing:border-box;" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.12)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
      ...
      <button onclick="jcDoRegister()" style="width:100%;background:linear-gradient(135deg,#CD9A48,#8b5a2b);color:#fff;border:none;border-radius:13px;padding:14px;font-weight:700;font-size:0.97rem;cursor:pointer;" onmouseover="this.style.opacity='0.88'" onmouseout="this.style.opacity='1'">
      ...
      <div style="text-align:center;margin-top:14px;">
        ...
        <button onclick="jcShowLogin()" style="background:none;border:none;color:#CD9A48;font-weight:700;font-size:0.82rem;cursor:pointer;text-decoration:underline;padding:0 4px;">Login</button>
      </div>
    </div>
    ...
    <div id="jcScreenLogin" style="display:none;padding:28px;">
      <div style="text-align:center;margin-bottom:20px;">
        <div style="font-size:0.72rem;color:#CD9A48;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;">Secure Payment</div>
      ...
      <button onclick="jcDoLogin()" style="width:100%;background:linear-gradient(135deg,#CD9A48,#8b5a2b);color:#fff;border:none;border-radius:13px;padding:14px;font-weight:700;font-size:0.97rem;cursor:pointer;" onmouseover="this.style.opacity='0.88'" onmouseout="this.style.opacity='1'">
        <i class="bi bi-shield-lock-fill me-2"></i>Login &amp; Send OTP
      </button>
      <div style="text-align:center;margin-top:14px;">
        <span style="font-size:0.82rem;color:#5C4335;">No account?</span>
        <button onclick="jcShowRegister()" style="background:none;border:none;color:#CD9A48;font-weight:700;font-size:0.82rem;cursor:pointer;text-decoration:underline;padding:0 4px;">Register</button>
      </div>
    </div>

    <!-- ── SCREEN 2: OTP ───────────────────────────────────────────── -->
    <div id="jcScreenOtp" style="display:none;padding:28px;text-align:center;">
      <div style="width:62px;height:62px;background:linear-gradient(135deg,#CD9A48,#8b5a2b);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
      ...
      <div style="font-size:0.83rem;color:#5C4335;margin-bottom:14px;">Enter the 6-digit OTP sent to<br><strong id="jcOtpMobileDisplay" style="color:#CD9A48;"></strong></div>
      ...
      <div id="jcSmsBubble" style="display:none; background:#f0f0f0;border-radius:16px;padding:12px 16px;margin-bottom:18px;text-align:left;position:relative;max-width:300px;margin-left:auto;margin-right:auto;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        <div style="font-size:0.65rem;color:#888;font-weight:600;margin-bottom:5px;display:flex;align-items:center;gap:5px;">
          <i class="bi bi-phone" style="color:#CD9A48;"></i> JazzCash SMS &mdash; just now
        </div>
        <div style="font-size:0.82rem;color:#222;">Your JazzCash OTP is: <strong id="jcSmsOtp" style="color:#CD9A48;font-size:1rem;letter-spacing:2px;">——</strong><br><span style="color:#888;font-size:0.72rem;">Valid for 5 minutes. Do not share.</span></div>
      </div>

      <!-- OTP Input boxes -->
      <div style="display:flex;gap:7px;justify-content:center;margin-bottom:18px;">
        <input class="otp-box" type="password" maxlength="1" style="width:44px;height:52px;border:1.5px solid #E6DCD2;border-radius:11px;text-align:center;font-size:1.4rem;font-weight:700;outline:none;color:#3D241C;background:#FFFDF9;" oninput="otpMove(this,0)" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.15)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
        <input class="otp-box" type="password" maxlength="1" style="width:44px;height:52px;border:1.5px solid #E6DCD2;border-radius:11px;text-align:center;font-size:1.4rem;font-weight:700;outline:none;color:#3D241C;background:#FFFDF9;" oninput="otpMove(this,1)" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.15)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
        <input class="otp-box" type="password" maxlength="1" style="width:44px;height:52px;border:1.5px solid #E6DCD2;border-radius:11px;text-align:center;font-size:1.4rem;font-weight:700;outline:none;color:#3D241C;background:#FFFDF9;" oninput="otpMove(this,2)" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.15)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
        <input class="otp-box" type="password" maxlength="1" style="width:44px;height:52px;border:1.5px solid #E6DCD2;border-radius:11px;text-align:center;font-size:1.4rem;font-weight:700;outline:none;color:#3D241C;background:#FFFDF9;" oninput="otpMove(this,3)" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.15)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
        <input class="otp-box" type="password" maxlength="1" style="width:44px;height:52px;border:1.5px solid #E6DCD2;border-radius:11px;text-align:center;font-size:1.4rem;font-weight:700;outline:none;color:#3D241C;background:#FFFDF9;" oninput="otpMove(this,4)" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.15)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
        <input class="otp-box" type="password" maxlength="1" style="width:44px;height:52px;border:1.5px solid #E6DCD2;border-radius:11px;text-align:center;font-size:1.4rem;font-weight:700;outline:none;color:#3D241C;background:#FFFDF9;" oninput="otpMove(this,5)" onfocus="this.style.borderColor='#CD9A48';this.style.boxShadow='0 0 0 3px rgba(205,154,72,0.15)'" onblur="this.style.borderColor='#E6DCD2';this.style.boxShadow='none'">
      </div>
      ...
      <button onclick="jcDoVerifyOtp()" style="width:100%;background:linear-gradient(135deg,#CD9A48,#8b5a2b);color:#fff;border:none;border-radius:13px;padding:14px;font-weight:700;font-size:0.97rem;cursor:pointer;" onmouseover="this.style.opacity='0.88'" onmouseout="this.style.opacity='1'">
    ...
    <div id="jcScreenSuccess" style="display:none;padding:40px 28px;text-align:center;">
      ...
      <div id="jcSuccessTxn" style="font-size:0.82rem;color:#CD9A48;font-weight:600;margin-bottom:6px;"></div>
      ...
      <div style="height:5px;background:#EAD7BF;border-radius:3px;overflow:hidden;">
        <div style="height:100%;background:linear-gradient(90deg,#CD9A48,#8b5a2b);border-radius:3px;animation:jcLoadBar 1.8s linear forwards;width:0;"></div>
      </div>
    </div>
...
<script>
  ...
  async function jcDoRegister() {
    ...
    try {
      const res = await fetch(BASE + 'index.php?page=jazzcash_register', { method:'POST', body:fd });
  ...
  async function jcDoLogin() {
    ...
    try {
      const res = await fetch(BASE + 'index.php?page=jazzcash_login', { method:'POST', body:fd });
      const data = await res.json();
      if (!data.success) {
  ...
  async function jcDoVerifyOtp() {
    ...
    try {
      const res = await fetch(BASE + 'index.php?page=jazzcash_verify_otp', { method:'POST', body:fd });
      const data = await res.json();
      if (!data.success) {
