...
<head>
    ...
    <style>
        ...
        .recovery-card.theme-default .back-link:hover { color: #CD9A48; }
...
<div class="recovery-card <?= $global_theme ?? 'theme-luxury' ?>">
    ...
    <?php if ($step === 'request'): ?>
    <!-- ── PAGE 1: EMAIL ── -->
        <p>Enter your registered email address and we'll send you a One-Time Password (OTP) to recover your account.</p>
        <form method="POST" action="<?= BASE_URL ?>index.php?page=customer_forgot_password_process">
            ...
            <button type="submit" class="btn-recover">Send OTP Code</button>
        </form>
        <a href="<?= BASE_URL ?>index.php?page=customer_login" class="back-link">
    ...
    <!-- ── PAGE 2: TIMER + OTP ── -->
        <p>A 6-digit OTP has been sent to <strong style="color:var(--accent-bronze)"><?= htmlspecialchars($_SESSION['reset_email_customer'] ?? '') ?></strong>. Enter it below before the timer runs out.</p>
        ...
        <form method="POST" action="<?= BASE_URL ?>index.php?page=customer_forgot_password_process" id="otp-form">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            ...
            <button type="submit" id="verify-btn" class="btn-recover">Verify OTP</button>
        </form>
        <a href="<?= BASE_URL ?>index.php?page=customer_forgot_password_process&action=cancel" class="back-link">
            Cancel &amp; Start Over
        </a>
        ...
        (function(){
            ...
            var cancel  = "<?= BASE_URL ?>index.php?page=customer_forgot_password_process&action=cancel";
