...
.auth-brand-title {
  ...
  font-size: 3rem;
  font-weight: 900;
  background: linear-gradient(135deg, #CD9A48 0%, #f0c97a 50%, #a67c37 100%);
...
.auth-brand-title span {
  background: linear-gradient(135deg, #f0c97a 0%, #CD9A48 100%);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
}
...
.auth-trust-pill i { color: #e8c07a; font-size: 0.8rem; }
...
.auth-tab-btn.active {
  background: linear-gradient(135deg, #CD9A48 0%, #a67c37 100%);
  color: #ffffff;
...
:root.theme-luxury .auth-tab-btn.active {
  background: linear-gradient(135deg, #CD9A48 0%, #a67c37 100%) !important;
...
:root.theme-luxury .auth-submit-btn:hover {
  background: linear-gradient(135deg, #d3ac63 0%, #a77628 100%) !important;
...
.auth-field:focus-within label { color: #CD9A48; }
...
.auth-field:focus-within .auth-input-icon { color: #CD9A48; }
...
.auth-eye-btn:hover { color: #CD9A48; }
...
.auth-forgot:hover { color: #CD9A48; text-decoration: underline; }
          ...
          <div class="auth-form-inner">
            ...
            <div class="auth-tab-content">
              ...
              <div class="auth-tab-pane <?= $activeTab === 'signin' ? 'active' : '' ?>" id="pane-signin">
                ...
                <form method="POST" action="<?= BASE_URL; ?>index.php?page=customer_login">
                  ...
                  <div class="auth-check-row">
                    ...
                    <a href="<?= BASE_URL; ?>index.php?page=customer_forgot_password" class="auth-forgot">
                      Forgot Password?
                    </a>
                  </div>
                  ...
                  <div class="auth-divider">or new here?
                    <a href="#" class="auth-forgot" id="goSignup" style="color:#CD9A48;">Create Account →</a>
                  </div>
                </form>
              </div>
              ...
              <div class="auth-tab-pane <?= $activeTab === 'signup' ? 'active' : '' ?>" id="pane-signup">
                ...
                <form method="POST" action="<?= BASE_URL; ?>index.php?page=customer_register" onsubmit="return validateRegisterForm()">
                  <input type="hidden" name="redirect" value="home">
                  <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? ''); ?>">
                  ...
                  <div class="auth-divider">already a member?
                    <a href="#" class="auth-forgot" id="goSignin" style="color:#CD9A48;">Sign In →</a>
                  </div>
                </form>
              </div>
...
(function() {
  ...
  function switchTo(tab) {
    if (tab === 'signup') {
      ...
      if (subtitle) subtitle.innerHTML = 'Join <span style="color:#CD9A48;font-weight:700;">Stitch Smart</span> — your luxury fashion journey begins here.';
    } else {
      ...
      if (subtitle) subtitle.innerHTML = 'Sign in to your <span style="color:#CD9A48;font-weight:700;">Stitch Smart</span> account to continue.';
    }
  }
...
(function() {
  ...
  style.textContent = `
    ...
    .fc-dot {
      ...
      background: radial-gradient(circle, #FFD700, #CD9A48);
