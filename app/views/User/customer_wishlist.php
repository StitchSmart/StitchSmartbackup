                ...
                <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                    <div class="card h-100 shadow wishlist-card">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="fw-bold mb-2" style="color:var(--accent-bronze,#CD9A48);font-size:1.1rem;">Rs. <?= number_format($entry['price']) ?></div>
                            <div class="mt-auto d-flex justify-content-between align-items-center gap-2">
                                <a href="<?= BASE_URL ?>/index.php?page=product_show&id=<?= $entry['product_id'] ?>" class="btn wishlist-view-btn btn-sm px-3">View</a>

                                <form method="post" action="<?= BASE_URL ?>/index.php?page=cart_add" style="display:inline;">
                                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                                    <input type="hidden" name="product_id" value="<?= (int)$entry['product_id'] ?>">
                                    <input type="hidden" name="qty" value="1">
                                    <input type="hidden" name="redirect_to" value="?page=customer_wishlist">
                                    <button type="submit" class="btn wishlist-addcart-btn" title="Add to cart" aria-label="Add to cart">
                                        <i class="bi bi-cart-plus"></i>
                                    </button>
                                </form>

                                <form method="post" action="<?= BASE_URL ?>/index.php?page=toggle_wishlist" style="display:inline;">
                                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                                    <input type="hidden" name="product_id" value="<?= (int)$entry['product_id'] ?>">
                                    <input type="hidden" name="redirect_to" value="?page=customer_wishlist">
...
<style>
    ...
    .wishlist-card:hover {
        ...
        border-color: rgba(193,154,78,0.38);
    }
    .wishlist-img-wrap {
    ...
    .wishlist-view-btn {
        border-radius: 999px;
        border: 1px solid rgba(193,154,78,0.52);
