...
<main class="design-main-shell">
  <section id="shopall-customizer" class="page active design-selection-section" aria-label="Shop All Catalog" style="margin-top: 2rem;">
    <a href="<?= BASE_URL ?>/index.php?page=design" class="back-btn" style="text-decoration: none;">← Back to Design Studio</a>
    ...
    <?php if (isset($totalPages) && $totalPages > 1): ?>
      <div class="pagination-container" style="display: flex; justify-content: center; gap: 10px; margin-top: 30px;">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
          <a href="<?= BASE_URL ?>/index.php?page=shopall_customizer&p=<?= $i ?>" 
             style="padding: 8px 16px; border-radius: 8px; text-decoration: none; <?= $i === $currentPage ? 'background: var(--accent-bronze, #c19a4e); color: #000; font-weight: bold;' : 'background: #eee; color: #333;' ?>">
            <?= $i ?>
          </a>
        <?php endfor; ?>
      </div>
...
Notes: ${notes}
    ...
    fetch('<?= BASE_URL ?>/index.php?page=send_design_inquiry', {
      method: 'POST',
      body: formData
    })
    .then(res => res.json())
