...
<style>
    ...
    :root.theme-luxury .brand-name span { color: #c19a4e !important; }
    ...
    :root.theme-default .brand-name span { color: #c89a5a !important; }
    ...
    <div class="container-fluid">
        <div class="navbar-wrapper">
            <a class="brand-logo" href="<?= BASE_URL ?>/index.php?page=home">
                <div style="width: 56px; height: 56px; border-radius: 8px; overflow: hidden; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border: 1px solid rgba(200, 154, 90, 0.4);">
                    <img src="<?= BASE_URL ?>/pictures/stitchsmart_luxury_icon.png" alt="StitchSmart Logo" style="width: 120%; height: 120%; object-fit: cover;">
                </div>
            ...
            <div class="navbar-right d-flex align-items-center gap-3">
                <form method="GET" action="<?= BASE_URL; ?>/index.php" class="search-form d-flex align-items-center">
                    <input type="hidden" name="page" value="allproducts">
            ...
            <div class="collapse d-lg-block w-100" id="categoryMenu">
                <div class="d-flex flex-nowrap align-items-center justify-content-between mt-2 mt-lg-0 w-100" style="padding: 10px 0; overflow-x: auto; white-space: nowrap; scrollbar-width: none;">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <a href="<?= BASE_URL; ?>/index.php?page=allproducts" class="btn btn-action btn-unified">
                            <i class="bi bi-grid me-2"></i> Shop All
                        </a>
                        <a href="<?= BASE_URL; ?>/index.php?page=page&slug=about-us" class="btn btn-action btn-unified">
                            <i class="bi bi-info-circle me-2"></i> About Us
                        </a>
                        <a href="<?= BASE_URL; ?>/index.php?page=sale" class="btn btn-action btn-unified btn-sale-highlight">
                            <i class="bi bi-tag-fill me-2"></i> Sale
                        </a>
                        <a href="<?= BASE_URL; ?>/index.php?page=design" class="btn btn-action btn-unified">
                            <i class="bi bi-pencil-square me-2"></i> Design
                        </a>
                        <a href="<?= BASE_URL; ?>/index.php?page=product_compare" class="btn btn-action position-relative btn-unified">
                            <i class="bi bi-bar-chart-line me-2"></i> Compare
                        </a>
                        <?php if (isset($_SESSION['customer_logged_in']) && $_SESSION['customer_logged_in'] === true): ?>
                            <a href="<?= BASE_URL; ?>/index.php?page=customer_orders" class="btn btn-action btn-unified" title="View Orders">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <?php if (isset($_SESSION['customer_logged_in']) && $_SESSION['customer_logged_in'] === true): ?>
                            <a href="<?= BASE_URL; ?>/index.php?page=customer_logout" class="btn btn-action btn-unified" title="Logout">
                                <i class="bi bi-box-arrow-right me-2"></i> Logout
                            </a>
                        <?php else: ?>
                            <a href="<?= BASE_URL; ?>/index.php?page=customer_login" class="btn btn-action btn-unified">
                                <i class="bi bi-lock me-2"></i> Login
                            </a>
                        <?php endif; ?>
                        <a href="<?= BASE_URL; ?>/index.php?page=customer_wishlist" class="btn btn-action position-relative btn-unified" title="Wishlist">
                            <i class="bi bi-heart me-2"></i> Wishlist
                        </a>
                        <a href="<?= BASE_URL; ?>/index.php?page=cart" class="btn btn-action cart-btn position-relative btn-unified" title="Shopping Cart">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    input.addEventListener("keyup", function () {
        ...
        fetch("<?= BASE_URL ?>/index.php?page=live_search&q=" + encodeURIComponent(query), { credentials: 'same-origin' })
            .then(res => res.json())
            .then(data => {
                let html = "";
                ...
                if (data.length > 0) {
                    data.forEach(item => {
                        let url = item.type === 'category'
                            ? '<?= BASE_URL ?>/index.php?page=allproducts&category_id=' + item.id
    ...
    input.addEventListener('focus', function() {
        ...
        if (query.length === 0) {
            ...
            fetch("<?= BASE_URL ?>/index.php?page=user_search_history", { credentials: 'same-origin' })
                .then(res => res.json())
                .then(data => {
