 ...
 <?php
        ...
        <?php endforeach; ?>
            ...
            <?php if (!empty($_SESSION['customer_id'])): ?>
                <form method="POST" action="<?= BASE_URL; ?>/index.php?page=wishlist_toggle" class="mt-3">
                    <input type="hidden" name="product_id" value="<?= (int)$product['id']; ?>">
                    <input type="hidden" name="redirect_to" value="<?= htmlspecialchars('?page=product_show&id=' . (int)$product['id'], ENT_QUOTES, 'UTF-8'); ?>">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? ''); ?>">
            ...
            <?php else: ?>
                <div class="mt-3">
                    <a href="<?= BASE_URL; ?>/index.php?page=customer_login" class="btn w-100 d-flex align-items-center justify-content-center" style="height: 52px; border-radius: 12px; border: 1px solid rgba(193, 154, 78, 0.7); background: rgba(255,255,255,0.08); color: #ffffff !important; font-weight: 700; text-decoration: none; transition: all 0.3s ease;">
...
.related-section {
  ...
  position: relative;
  overflow: hidden;
  border-top: 1px solid rgba(193,154,78,0.15);
}
.related-section::before {
  ...
  background: radial-gradient(ellipse at top, rgba(193,154,78,0.08) 0%, transparent 70%);
  pointer-events: none;
}
.related-section .section-eyebrow {
  ...
  background: linear-gradient(135deg, rgba(193,154,78,0.1) 0%, rgba(166,124,55,0.15) 100%);
  border: 1px solid rgba(193,154,78,0.3);
  padding: 6px 20px;
  border-radius: 50px;
  margin-bottom: 20px;
  box-shadow: 0 4px 15px rgba(193, 154, 78, 0.1);
}
...
.related-section .section-heading span {
  color: transparent;
  background: linear-gradient(135deg, #c19a4e, #8b6914);
...
.rel-card:hover {
  ...
  box-shadow: 0 20px 50px rgba(0,0,0,0.08), 0 0 0 2px rgba(193,154,78,0.4);
}
.rel-card-img {
  position: relative;
  overflow: hidden;
...
.rel-card-btn {
  ...
  text-transform: uppercase;
  letter-spacing: 1.5px;
  box-shadow: 0 4px 15px rgba(193, 154, 78, 0.3);
}
.rel-card-btn:hover {
  background: #1a0f0a;
  color: #c19a4e;
...
.empty-state-bg-shapes {
  ...
  width: 450px;
  height: 450px;
  background: radial-gradient(circle, rgba(193, 154, 78, 0.15) 0%, rgba(0,0,0,0) 70%);
  top: -100px;
  left: -100px;
...
.empty-state-bg-shapes.two {
  ...
  background: radial-gradient(circle, rgba(166, 124, 55, 0.1) 0%, rgba(0,0,0,0) 70%);
...
.empty-state-content {
  ...
  max-width: 650px;
  text-align: center;
  box-shadow: 0 15px 40px rgba(193,154,78,0.08), 0 2px 8px rgba(0,0,0,0.04);
...
.empty-state-content .icon-container {
  ...
  align-items: center;
  justify-content: center;
  box-shadow: 0 15px 35px rgba(193,154,78,0.35);
...
.empty-state-content .icon-container::after {
  ...
  inset: -10px;
  border-radius: 50%;
  border: 1px dashed rgba(193, 154, 78, 0.5);
  animation: spinBorder 15s linear infinite;
}
@keyframes pulseLuxury {
  0% { box-shadow: 0 0 0 0 rgba(193, 154, 78, 0.4), 0 15px 35px rgba(193,154,78,0.25); }
  70% { box-shadow: 0 0 0 20px rgba(193, 154, 78, 0), 0 15px 35px rgba(193,154,78,0.25); }
  100% { box-shadow: 0 0 0 0 rgba(193, 154, 78, 0), 0 15px 35px rgba(193,154,78,0.25); }
}
...
.btn-empty-state {
  ...
  text-transform: uppercase;
  border: none;
  box-shadow: 0 8px 25px rgba(193,154,78,0.35);
}
      ...
      <div class="col rel-card-wrapper">
        <div class="rel-card">
          <div class="rel-card-img">
            ...
            <a href="<?= BASE_URL ?>/index.php?page=product_show&id=<?= (int)$rel['id'] ?>" class="rel-card-quick-view">
          ...
          <div class="rel-card-body">
            ...
            <form method="POST" action="<?= BASE_URL ?>/index.php?page=cart_add" style="margin-top: auto;">
              <input type="hidden" name="product_id" value="<?= (int)$rel['id'] ?>">
              <input type="hidden" name="qty" value="1">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
              <input type="hidden" name="redirect_to" value="?page=product_show&id=<?= (int)$product['id'] ?>">
    ...
    <div class="empty-state-wrapper">
      ...
      <div class="empty-state-content">
        ...
        <?php if (empty($_SESSION['customer_id'])): ?>
          ...
          <a href="<?= BASE_URL ?>/index.php?page=customer_login" class="btn-empty-state">Sign In & Discover <i class="fas fa-arrow-right ms-2"></i></a>
        <?php else: ?>
          ...
          <a href="<?= BASE_URL ?>/index.php?page=products" class="btn-empty-state">Explore Collection <i class="fas fa-arrow-right ms-2"></i></a>
        <?php endif; ?>
      </div>
    </div>
    <?php endif; ?>
