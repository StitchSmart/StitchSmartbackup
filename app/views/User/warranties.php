...
if ($theme === 'theme-luxury') {
    $cardAccent = '#D4AF37'; // Gold
    $cardDark = '#111111';   // Black
    $cardLight = '#1a1a1a';  // Dark Gray
...
.w-page {
    ...
    color: <?= $theme === 'theme-luxury' ? '#fff' : '#111' ?>;
}
...
.hero-text {
    ...
    line-height: 1.6;
    margin-bottom: 40px;
    color: <?= $theme === 'theme-luxury' ? '#ccc' : '#444' ?>;
}
.hero-icons {
    display: flex;
    gap: 15px;
...
<div class="w-page">
    ...
    <div class="container pb-5" id="claim-section">
        ...
        <?php if (!empty($claims)): ?>
            <div class="mt-5 pt-4">
                <div class="card border-0 mx-auto" style="max-width: 900px; border-radius: 12px; background: <?= $cardLight ?>; box-shadow: 0 4px 20px rgba(0,0,0,0.15); overflow: hidden;">
                    
                    <!-- Header -->
                    <div class="d-flex align-items-center p-4" style="background: <?= $cardLight ?>; border-bottom: 1px solid <?= $tableBorder ?>;">
                        <div style="width: 50px; height: 50px; background: <?= $cardAccent ?>; color: <?= $textLight ?>; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; margin-right: 20px;">
                            <i class="bi bi-folder2-open"></i>
                        </div>
                        <div>
                            <h3 class="mb-0" style="font-family: 'Playfair Display', serif; font-weight: 800; color: <?= $textDark ?>; font-size: 1.6rem;">Claim Summary</h3>
                            <p class="mb-0" style="color: <?= $theme === 'theme-luxury' ? '#aaa' : '#2e5a31' ?>; font-size: 0.9rem; font-weight: 600;">Record of your submitted warranty claims</p>
                        </div>
                    </div>

                    <!-- Table Section -->
                    <div class="p-4" style="background: <?= $cardLight ?>;">
                        <div style="border: 1px solid <?= $tableBorder ?>; border-radius: 8px; overflow: hidden;">
                            <div class="p-2 px-3 fw-bold text-uppercase" style="background: <?= $cardAccent ?>; color: <?= $textLight ?>; font-size: 0.85rem; letter-spacing: 1px;">
                                Claim Summary
                            </div>
                            <table class="table table-borderless mb-0 align-middle" style="background: <?= $cardLight ?>;">
                                <thead>
                                    <tr style="border-bottom: 2px solid <?= $tableBorder ?>;">
                                        <th class="text-center text-uppercase" style="color: <?= $theme === 'theme-luxury' ? '#888' : '#666' ?>; font-size: 0.75rem; letter-spacing: 1px; width: 20%;">Date</th>
                                        <th class="text-uppercase" style="color: <?= $theme === 'theme-luxury' ? '#888' : '#666' ?>; font-size: 0.75rem; letter-spacing: 1px; width: 35%;">Activity / Issue</th>
                                        <th class="text-center text-uppercase" style="color: <?= $theme === 'theme-luxury' ? '#888' : '#666' ?>; font-size: 0.75rem; letter-spacing: 1px; width: 25%;">Warranty Ref</th>
                                        <th class="text-center text-uppercase" style="color: <?= $theme === 'theme-luxury' ? '#888' : '#666' ?>; font-size: 0.75rem; letter-spacing: 1px; width: 20%;">Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php 
                                    ...
                                    foreach ($claims as $c): 
                                        $rowCount++;
                                        $rowBg = ($rowCount % 2 == 0) ? $rowBgEven : $rowBgOdd;
                                    ?>
                                        <tr style="background-color: <?= $rowBg ?>; border-bottom: 1px solid <?= $tableBorder ?>;">
                                            <td class="text-center" style="font-size: 0.9rem; color: <?= $textDark ?>; padding: 15px 10px;">
                                                <?= date('m/d/Y', strtotime($c['created_at'])) ?>
                                            </td>
                                            <td style="font-size: 0.9rem; color: <?= $textDark ?>; padding: 15px 10px;">
                                                <div class="fw-bold mb-1">Claim Initiated (ID: #<?= $c['id'] ?>)</div>
                                                <div style="color: <?= $theme === 'theme-luxury' ? '#bbb' : '#666' ?>; font-size: 0.85rem; line-height: 1.4;">
                                                    <?= htmlspecialchars($c['issue_description'] ?? 'No description provided.') ?>
                                                </div>
                                                <?php if(!empty($c['admin_notes'])): ?>
                                                    <div class="mt-2 p-2 rounded" style="background: <?= $theme === 'theme-luxury' ? 'rgba(212, 175, 55, 0.1)' : 'rgba(166, 124, 82, 0.1)' ?>; border-left: 2px solid <?= $cardAccent ?>; font-size: 0.8rem; color: <?= $textDark ?>;">
                                                        <strong>Reply:</strong> <?= htmlspecialchars($c['admin_notes']) ?>
                                                    </div>
                                                <?php endif; ?>
                                            </td>
                                            <td class="text-center" style="padding: 15px 10px;">
                                                <span style="font-family: monospace; font-size: 0.85rem; font-weight: 700; color: <?= $cardAccent ?>; background: <?= $theme === 'theme-luxury' ? 'rgba(212, 175, 55, 0.1)' : 'rgba(166, 124, 82, 0.1)' ?>; padding: 4px 10px; border-radius: 4px;">
